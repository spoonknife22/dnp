...
const ExpiredNominationModal = ({ parcel, props, userManaBalance }) => {

  const {drizzleState, drizzle} = props;
  const tokenId = parcel[0].nft.parcel.tokenId;
      ...
      <>   
        ...
        <Modal className="nomintation-modal" show={show} onHide={handleClose}>
          <Modal.Header className="text-center justify-content-center" closeButton>
            ...
            <div className="current-price-title">Current Price:&nbsp;{Web3.utils.fromWei(`${parcel[0].nft.activeOrder.price}`,'ether')}&nbsp;MANA</div>
        ...
        <style jsx>
          {`  
              ...
              .card-footer-btn{background:#e6e6e6;color:#000000}
              .btn-lg{border-radius: 0rem;
                border-top-left-radius: 0rem;
                border-top-right-radius: 0rem;
                border-bottom-right-radius: .5rem;
                border-bottom-left-radius: .5rem;}
              .card-footer-btn:hover{background:#404040;color:#ffffff;}
...
export default connect(null, mapDispatchToProps)(ExpiredNominationModal);
